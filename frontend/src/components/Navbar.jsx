        ...
        <div className="py-4 px-4 sm:px-8 lg:px-12">
          <div className='flex justify-between'>
            <div className='flex items-center'>
              <Link to='/'> <img src={BlogCraftLogo} alt="" className=" h-10" /> </Link>
            </div>
            ...
            <div className={` md:relative md:flex md:flex-row md:justify-end md:gap-7 md:items-center md:pb-0 md:pr-0  text-black text-lg font-semibold   ${menuOpen ? 'flex flex-col bg-white shadow-md items-end gap-4 absolute top-[100%] left-0 w-full pb-4 pr-4 sm:pr-8' : 'hidden'}`}>
              ...
              {isAuth ? (
                <>
                  <Link to='/myblogs' className='hover:text-blue-600 transition-colors'>myblogs</Link>
                  <Link to='/createBlog' className='hover:text-blue-600 transition-colors'>createblog</Link>
                  <Link to='/' onClick={HandleSignout} className='hover:text-blue-600 transition-colors'>Signout</Link>
                </>
              ) : (
                <>
